{
  ...
  configFile = tomlFormat.generate "shpool-config.toml" ({shared = {};} // cfg.settings);
in {
  options.services.shpool = {
      # seems like shpool and nu hate each other. disabled for now.
  ...
  config = lib.mkIf cfg.enable {
    xdg.configFile."shpool/config.toml".source = configFile;
