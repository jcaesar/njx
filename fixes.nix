final: prev: {
  ...
  libselinux = prev.libselinux.overrideAttrs (orig:
    if final.stdenv.hostPlatform.isStatic
    then {
      patches =
  ...
  brscan5 = prev.brscan5.overrideAttrs (orig: {
    postPatch = ''
      ${orig.postPatch}
      # SYSFS removed with udev-173, see pkgs/by-name/br/brscan4/udev_rules_type1.nix
      sed -i -e s/SYSFS/ATTR/g opt/brother/scanner/brscan5/udev-rules/*.rules
    '';
  });
}
